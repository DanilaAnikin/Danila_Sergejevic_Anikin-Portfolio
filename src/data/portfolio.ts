...
export const projects = [
  ...
  {
    title: "Perennial",
    href: "https://farm.anikin.cz/",
    image: "/projects/perennial.png",
    eyebrow: l(
      "Autonomous software factory",
      "Autonomní továrna na software",
      "Автономная фабрика софта",
      "Autonome Software-Fabrik",
      "Fábrica de software autónoma",
      "Usine logicielle autonome",
    ),
    description: l(
      "A swarm of agents that turns a one-line wish into merged code: a manager writes the spec, an architect emits a dependency graph of tasks, parallel workers build each one in its own git worktree, and a judge plus an end-to-end tester gate the result before it opens a pull request.",
      "Roj agentů, který z jednovětého přání udělá sloučený kód: manažer sepíše zadání, architekt z něj vytvoří graf závislostí úkolů, workery je staví paralelně každý ve vlastním git worktree a soudce s koncovým testerem výsledek propustí, teprve pak vzniká pull request.",
      "Рой агентов, превращающий однострочное пожелание в смёрженный код: менеджер пишет спецификацию, архитектор строит граф зависимостей задач, воркеры делают каждую в своём git worktree, а судья и e2e-тестер пропускают результат до открытия pull request.",
      "Ein Agenten-Schwarm, der aus einem Einzeiler gemergten Code macht: Ein Manager schreibt die Spezifikation, ein Architekt erzeugt einen Abhängigkeitsgraphen, Worker bauen jede Aufgabe im eigenen git-Worktree, und ein Judge plus E2E-Tester geben das Ergebnis frei, bevor ein Pull Request entsteht.",
      "Un enjambre de agentes que convierte un deseo de una línea en código fusionado: un gestor escribe la especificación, un arquitecto genera un grafo de dependencias, los workers construyen cada tarea en su propio git worktree y un juez más un tester e2e aprueban el resultado antes del pull request.",
      "Un essaim d'agents qui transforme un souhait d'une ligne en code fusionné : un manager rédige la spécification, un architecte produit un graphe de dépendances, des workers construisent chaque tâche dans son propre git worktree, et un juge plus un testeur e2e valident avant l'ouverture de la pull request.",
    ),
    impact: l(
      "41,377 agent attempts across 7 repositories for $44 in total spend — held there by per-attempt, per-project and monthly caps that park work instead of overrunning.",
      "41 377 pokusů agentů přes 7 repozitářů za celkových $44 — drží to stropy na pokus, na projekt a na měsíc, které práci odloží, místo aby přestřelily rozpočet.",
      "41 377 попыток агентов в 7 репозиториях за $44 суммарно — удерживают лимиты на попытку, на проект и на месяц, которые ставят работу на паузу вместо перерасхода.",
      "41.377 Agentenversuche über 7 Repositories für insgesamt 44 $ — gehalten von Limits pro Versuch, pro Projekt und pro Monat, die Arbeit parken statt das Budget zu sprengen.",
      "41 377 intentos de agentes en 7 repositorios por 44 $ en total — sostenido por límites por intento, por proyecto y mensuales que aparcan el trabajo en vez de excederse.",
      "41 377 tentatives d'agents sur 7 dépôts pour 44 $ au total — tenu par des plafonds par tentative, par projet et mensuels qui mettent le travail en pause au lieu de déraper.",
    ),
    stack: ["TypeScript", "Next.js", "Turborepo", "Drizzle / PostgreSQL", "pgmq", "LiteLLM", "Docker"],
    accent: "violet",
  },
  {
    title: "Frem",
    href: "https://www.youtube.com/@fremmmm",
    image: "/projects/frem.png",
    eyebrow: l(
      "Autonomous video pipeline",
      "Autonomní výroba videí",
      "Автономный видеоконвейер",
      "Autonome Video-Pipeline",
      "Pipeline de vídeo autónoma",
      "Pipeline vidéo autonome",
    ),
    description: l(
      "A self-hosted pipeline that takes a video from script to published upload: LLM-written scripts, narration with word-level timestamps, generated imagery placed against those timestamps, an ffmpeg render on the server's integrated GPU, then a scheduled upload through the YouTube Data API.",
      "Self-hosted linka, která video dotáhne od scénáře až po nahrání: scénář píše LLM, namluvení má časové značky po slovech, podle nich se rozmístí vygenerované obrázky, ffmpeg to vyrenderuje na integrované GPU serveru a YouTube Data API to nahraje na naplánovaný termín.",
      "Self-hosted конвейер, ведущий видео от сценария до публикации: сценарий пишет LLM, озвучка имеет по-словные таймкоды, по ним расставляются сгенерированные изображения, ffmpeg рендерит на встроенном GPU сервера, а YouTube Data API загружает по расписанию.",
      "Eine self-hosted Pipeline vom Skript bis zum veröffentlichten Upload: LLM-geschriebene Skripte, Sprecherstimme mit wortgenauen Zeitmarken, daran ausgerichtete generierte Bilder, ein ffmpeg-Render auf der integrierten GPU des Servers und ein geplanter Upload über die YouTube Data API.",
      "Una pipeline self-hosted que lleva un vídeo del guion a la publicación: guiones escritos por un LLM, narración con marcas de tiempo por palabra, imágenes generadas colocadas según esas marcas, render con ffmpeg en la GPU integrada del servidor y subida programada vía YouTube Data API.",
      "Une pipeline self-hosted qui mène la vidéo du script à la publication : scripts écrits par un LLM, narration horodatée mot à mot, images générées placées sur ces repères, rendu ffmpeg sur le GPU intégré du serveur, puis mise en ligne planifiée via l'API YouTube Data.",
    ),
    impact: l(
      "277 published videos and 60,576 views on the channel — the card opens it.",
      "277 zveřejněných videí a 60 576 zhlédnutí na kanálu — karta ho otevře.",
      "277 опубликованных видео и 60 576 просмотров на канале — карточка его открывает.",
      "277 veröffentlichte Videos und 60.576 Aufrufe auf dem Kanal — die Karte öffnet ihn.",
      "277 vídeos publicados y 60 576 visualizaciones en el canal — la tarjeta lo abre.",
      "277 vidéos publiées et 60 576 vues sur la chaîne — la carte l'ouvre.",
    ),
    stack: ["TypeScript", "Next.js", "Python", "ffmpeg / VAAPI", "PostgreSQL", "YouTube Data API"],
    accent: "coral",
  },
  {
    title: "Mimic",
    href: "https://mimic.anikin.cz/",
    image: "/projects/mimic.png",
    eyebrow: l(
      "Copy-trading platform",
      "Platforma na kopírování obchodů",
      "Платформа копи-трейдинга",
      "Copy-Trading-Plattform",
      "Plataforma de copy trading",
      "Plateforme de copy trading",
    ),
    description: l(
      "Users follow a trader and every trade is mirrored onto their own exchange account. A single detection worker polls each watched portfolio once for the whole platform and fans trades out per follower, with encrypted agent keys and per-user isolation so one failure never stalls anyone else.",
      "Uživatel sleduje obchodníka a každý jeho obchod se zrcadlí na vlastní účet. Jeden detekční worker se ptá každého sledovaného portfolia jednou za celou platformu a obchody rozesílá jednotlivým sledujícím; klíče jsou šifrované a uživatelé oddělení, takže jeden pád nezastaví ostatní.",
      "Пользователь подписывается на трейдера, и каждая сделка зеркалится на его собственный счёт. Один детектирующий воркер опрашивает каждый отслеживаемый портфель один раз за всю платформу и раздаёт сделки подписчикам; ключи зашифрованы, пользователи изолированы, поэтому один сбой не останавливает остальных.",
      "Nutzer folgen einem Trader, und jeder Trade wird auf ihr eigenes Börsenkonto gespiegelt. Ein einzelner Detection-Worker fragt jedes beobachtete Portfolio einmal für die gesamte Plattform ab und verteilt die Trades pro Follower — mit verschlüsselten Schlüsseln und Isolation je Nutzer, sodass ein Fehler nie alle blockiert.",
      "Los usuarios siguen a un trader y cada operación se replica en su propia cuenta. Un único worker de detección consulta cada cartera vigilada una vez para toda la plataforma y reparte las operaciones por seguidor, con claves cifradas y aislamiento por usuario para que un fallo no detenga a los demás.",
      "Les utilisateurs suivent un trader et chaque transaction est répliquée sur leur propre compte. Un unique worker de détection interroge chaque portefeuille surveillé une seule fois pour toute la plateforme et redistribue les transactions par abonné, avec des clés chiffrées et une isolation par utilisateur.",
    ),
    impact: l(
      "Web app plus native Android and iOS clients, with a watchdog that restarts the worker on a stale cycle.",
      "Webová aplikace plus nativní klienti pro Android a iOS, s hlídačem, který worker restartuje, když se zasekne cyklus.",
      "Веб-приложение плюс нативные клиенты для Android и iOS, со сторожем, который перезапускает воркер при зависшем цикле.",
      "Web-App plus native Android- und iOS-Clients, mit einem Watchdog, der den Worker bei hängendem Zyklus neu startet.",
      "Aplicación web más clientes nativos de Android e iOS, con un watchdog que reinicia el worker si el ciclo se atasca.",
      "Application web plus clients natifs Android et iOS, avec un watchdog qui redémarre le worker en cas de cycle bloqué.",
    ),
    stack: ["Next.js", "React", "TypeScript", "Drizzle / PostgreSQL", "Kotlin", "SwiftUI"],
    accent: "cyan",
  },
  {
    title: "Faktura",
    href: "https://faktura.anikin.cz/",
    image: "/projects/faktura.png",
    eyebrow: l(
      "Czech invoicing system",
      "Fakturační systém",
    ...
    description: l(
      "Invoicing built to Czech rules: issued documents are immutable, with number series, VAT regimes, advance and tax-payment documents, central-bank exchange rates, and CZ/EN PDF output carrying SPAYD and EPC payment QR codes.",
      "Fakturace postavená na českých pravidlech: vystavené doklady jsou neměnné, k tomu číselné řady, režimy DPH, zálohové a daňové doklady, kurzy ČNB a PDF v češtině i angličtině s platebními QR kódy SPAYD a EPC.",
      "Выставление счетов по чешским правилам: выпущенные документы неизменяемы, есть номерные серии, режимы НДС, авансовые и налоговые документы, курсы центробанка и PDF на чешском и английском с платёжными QR-кодами SPAYD и EPC.",
      "Rechnungsstellung nach tschechischen Regeln: ausgestellte Belege sind unveränderlich, dazu Nummernkreise, Umsatzsteuer-Regime, Anzahlungs- und Steuerbelege, Zentralbank-Wechselkurse und PDFs auf Tschechisch und Englisch mit SPAYD- und EPC-Zahlungs-QR-Codes.",
      "Facturación según las normas checas: los documentos emitidos son inmutables, con series de numeración, regímenes de IVA, documentos de anticipo y fiscales, tipos de cambio del banco central y PDF en checo e inglés con códigos QR de pago SPAYD y EPC.",
      "Facturation conforme aux règles tchèques : les documents émis sont immuables, avec séries de numérotation, régimes de TVA, documents d'acompte et fiscaux, taux de change de la banque centrale et PDF en tchèque et en anglais portant des QR codes de paiement SPAYD et EPC.",
    ),
    impact: l(
      "261 tests plus a browser smoke run against a real database; separate PDF and job workers, automated backup, retention and restore drills.",
      "261 testů plus prohlížečový smoke test proti skutečné databázi; samostatné workery na PDF a úlohy, automatické zálohy, retence a zkoušky obnovy.",
      "261 тест плюс браузерный smoke-прогон по реальной базе; отдельные воркеры для PDF и задач, автоматические бэкапы, retention и учения по восстановлению.",
      "261 Tests plus ein Browser-Smoke-Lauf gegen eine echte Datenbank; getrennte PDF- und Job-Worker, automatische Backups, Retention und Restore-Übungen.",
      "261 pruebas más una ejecución smoke en navegador contra una base de datos real; workers separados de PDF y trabajos, copias automáticas, retención y simulacros de restauración.",
      "261 tests plus un smoke run navigateur sur une vraie base ; workers PDF et jobs séparés, sauvegardes automatiques, rétention et exercices de restauration.",
    ),
    stack: ["Python", "Django 5.2", "PostgreSQL 17", "WeasyPrint", "pytest", "Playwright", "Docker"],
    accent: "blue",
  },
  {
    title: "Remote Terminal",
    href: "https://terminal.anikin.cz/",
    ...
    description: l(
      "A multi-terminal workspace for a home server: real PTYs over a binary WebSocket, tmux-backed sessions that survive app restarts, split panes, session recording with full-text search across past sessions, drag-and-drop file transfer and inline images.",
      "Vícepanelový terminál pro domácí server: skutečná PTY přes binární WebSocket, sezení nad tmuxem přežijí restart aplikace, dělené panely, nahrávání sezení s fulltextovým hledáním v historii, přenos souborů přetažením a obrázky přímo v terminálu.",
      "Многотерминальное рабочее место для домашнего сервера: настоящие PTY по бинарному WebSocket, сессии на tmux переживают перезапуск приложения, разделённые панели, запись сессий с полнотекстовым поиском по истории, передача файлов перетаскиванием и картинки прямо в терминале.",
      "Ein Multi-Terminal-Arbeitsplatz für den Heimserver: echte PTYs über binäres WebSocket, tmux-gestützte Sessions, die App-Neustarts überleben, geteilte Panes, Session-Aufzeichnung mit Volltextsuche über frühere Sessions, Drag-and-drop-Dateitransfer und Inline-Bilder.",
      "Un espacio multi-terminal para un servidor doméstico: PTY reales sobre WebSocket binario, sesiones con tmux que sobreviven a los reinicios, paneles divididos, grabación de sesiones con búsqueda de texto completo, transferencia de archivos por arrastre e imágenes en línea.",
      "Un espace multi-terminal pour un serveur domestique : de vrais PTY sur WebSocket binaire, des sessions tmux qui survivent aux redémarrages, des panneaux divisés, l'enregistrement de session avec recherche plein texte, le transfert de fichiers par glisser-déposer et des images en ligne.",
    ),
    impact: l(
      "Passkeys, TOTP, role-based accounts and expiring read-only share links; bound to loopback only and published behind an identity proxy.",
      "Passkeys, TOTP, účty podle rolí a sdílecí odkazy s platností jen pro čtení; služba poslouchá pouze na loopbacku a ven jde přes identitní proxy.",
      "Passkeys, TOTP, ролевые аккаунты и истекающие ссылки только для чтения; служба слушает лишь loopback и публикуется через identity-прокси.",
      "Passkeys, TOTP, rollenbasierte Konten und ablaufende Nur-Lese-Freigabelinks; nur an Loopback gebunden und hinter einem Identity-Proxy veröffentlicht.",
      "Passkeys, TOTP, cuentas por rol y enlaces de solo lectura que caducan; escucha únicamente en loopback y se publica tras un proxy de identidad.",
      "Passkeys, TOTP, comptes par rôle et liens de partage en lecture seule expirants ; lié uniquement au loopback et publié derrière un proxy d'identité.",
    ),
    stack: ["TypeScript", "Node.js", "WebSocket", "node-pty", "xterm.js", "React", "tmux"],
    accent: "green",
  },
  {
    ...
    eyebrow: l(
      ...
      "Autonomní e-mailový agent",
      "Автономный email-агент",
      "Autonomer E-Mail-Agent",
      "Agente de correo autónomo",
      "Agent e-mail autonome",
    ),
    description: l(
      "An agent that runs cold outreach and inbound replies unattended: it discovers local businesses, generates a per-brand audit, writes each message with an LLM and sends it through my own mail service under hard daily caps.",
      "Agent, který bez dozoru vede oslovování i odpovídání: najde místní firmy, vygeneruje audit podle značky, každou zprávu napíše pomocí LLM a odešle ji přes mou vlastní odesílací službu pod pevnými denními stropy.",
      "Агент, который без присмотра ведёт холодную рассылку и ответы: находит локальные компании, готовит аудит под бренд, пишет каждое письмо с помощью LLM и отправляет через мой собственный почтовый сервис в рамках жёстких дневных лимитов.",
      "Ein Agent, der Kaltakquise und eingehende Antworten unbeaufsichtigt erledigt: Er findet lokale Betriebe, erzeugt ein markenspezifisches Audit, schreibt jede Nachricht per LLM und versendet sie über meinen eigenen Maildienst unter harten Tageslimits.",
      "Un agente que gestiona la prospección en frío y las respuestas entrantes sin supervisión: descubre negocios locales, genera una auditoría por marca, escribe cada mensaje con un LLM y lo envía por mi propio servicio de correo con límites diarios estrictos.",
      "Un agent qui mène la prospection à froid et les réponses entrantes sans supervision : il découvre des entreprises locales, génère un audit par marque, rédige chaque message avec un LLM et l'envoie via mon propre service d'e-mail sous des plafonds quotidiens stricts.",
    ),
    impact: l(
      "1,110 emails across six local-business brands since July 2026, with threaded follow-ups, unsubscribe handling and escalation to a human after four automated replies.",
      "1 110 e-mailů napříč šesti lokálními značkami od července 2026, s navazujícími připomínkami ve vlákně, řešením odhlášení a předáním člověku po čtyřech automatických odpovědích.",
      "1 110 писем по шести локальным брендам с июля 2026 года, с последующими напоминаниями в треде, обработкой отписок и передачей человеку после четырёх автоответов.",
      "1.110 E-Mails über sechs lokale Marken seit Juli 2026, mit Follow-ups im Thread, Abmelde-Handling und Eskalation an einen Menschen nach vier automatischen Antworten.",
      "1110 correos en seis marcas locales desde julio de 2026, con seguimientos en el hilo, gestión de bajas y escalado a una persona tras cuatro respuestas automáticas.",
      "1 110 e-mails sur six marques locales depuis juillet 2026, avec relances dans le fil, gestion des désabonnements et escalade vers un humain après quatre réponses automatiques.",
    ),
    stack: ["Python", "PostgreSQL", "systemd", "Anthropic API", "IMAP / SMTP", "Self-hosted ESP"],
    accent: "yellow",
  },
  {
    title: "Mail agent",
    href: "https://github.com/DanilaAnikin",
    image: "/projects/mailagent.png",
    eyebrow: l(
      "Inbox agent with a human gate",
      "Agent pošty se schválením",
      "Почтовый агент с подтверждением",
      "Posteingangs-Agent mit Freigabe",
      "Agente de bandeja con aprobación",
      "Agent de boîte mail avec validation",
    ),
    description: l(
      "An agent that watches fifteen mailboxes and gives each incoming message its own headless LLM session. The model gets no built-in tools at all and can reach only that one thread, through a token-scoped MCP server; whether to reply, ignore or escalate is then decided in code behind a policy gate.",
      "Agent hlídá patnáct schránek a každou příchozí zprávu dá vlastnímu běhu LLM bez rozhraní. Model nedostane žádné vestavěné nástroje a dosáhne jen na to jedno vlákno, a to přes MCP server omezený tokenem; jestli odpovědět, nechat být, nebo předat člověku, rozhoduje až kód za pravidlovou bránou.",
      "Агент следит за пятнадцатью ящиками и на каждое входящее письмо запускает отдельную headless-сессию LLM. У модели нет ни одного встроенного инструмента, и дотянуться она может только до этой переписки — через MCP-сервер, ограниченный токеном; отвечать, игнорировать или эскалировать решает уже код за политиками.",
      "Ein Agent überwacht fünfzehn Postfächer und gibt jeder eingehenden Nachricht eine eigene Headless-LLM-Session. Das Modell bekommt überhaupt keine eingebauten Werkzeuge und erreicht nur genau diesen Thread über einen token-begrenzten MCP-Server; ob geantwortet, ignoriert oder eskaliert wird, entscheidet danach der Code hinter einem Policy-Gate.",
      "Un agente vigila quince buzones y da a cada mensaje entrante su propia sesión headless de LLM. El modelo no recibe ninguna herramienta integrada y solo alcanza ese hilo, mediante un servidor MCP limitado por token; si responder, ignorar o escalar lo decide después el código tras una barrera de políticas.",
      "Un agent surveille quinze boîtes mail et attribue à chaque message entrant sa propre session LLM headless. Le modèle ne reçoit aucun outil intégré et n'atteint que ce fil, via un serveur MCP limité par jeton ; répondre, ignorer ou escalader est ensuite décidé par le code derrière une barrière de règles.",
    ),
    impact: l(
      "Nothing leaves on its own: the proposed reply lands in Telegram, where I approve, edit or reject it first.",
      "Samo od sebe neodejde nic: navržená odpověď přijde na Telegram a tam ji nejdřív schválím, upravím, nebo zamítnu.",
      "Само по себе ничего не уходит: предложенный ответ приходит в Telegram, и там я его сначала одобряю, правлю или отклоняю.",
      "Von allein geht nichts raus: Die vorgeschlagene Antwort landet in Telegram, wo ich sie zuerst freigebe, ändere oder ablehne.",
      "Nada sale por su cuenta: la respuesta propuesta llega a Telegram, donde primero la apruebo, edito o rechazo.",
      "Rien ne part tout seul : la réponse proposée arrive dans Telegram, où je l'approuve, la modifie ou la refuse d'abord.",
    ),
    stack: ["TypeScript", "Node.js", "MCP", "SQLite", "Zod", "Docker", "Telegram Bot API"],
    accent: "blue",
  },
  {
    title: "Jarvis",
    href: "https://github.com/DanilaAnikin/jarvis",
    image: "/projects/jarvis.png",
    eyebrow: l(
      "Personal voice AI agent",
      "Osobní hlasový AI agent",
      "Персональный голосовой AI-агент",
      "Persönlicher KI-Sprachagent",
      "Agente de voz AI personal",
      "Agent vocal AI personnel",
    ),
    description: l(
      "A hands-free agent for Linux: a wake-word voice loop with speech-to-text and streaming speech output, MCP tool servers for shell, browser, filesystem and desktop control, and persistent memory across conversations.",
      "Agent pro Linux ovládaný hlasem: smyčka s aktivačním slovem, převod řeči na text a průběžně streamovaná odpověď, MCP nástrojové servery pro shell, prohlížeč, souborový systém i ovládání plochy, a paměť, která přetrvává mezi hovory.",
      "Агент для Linux без рук: цикл с активационным словом, распознавание речи и потоковый синтез ответа, MCP-серверы инструментов для shell, браузера, файловой системы и управления рабочим столом, а также память между разговорами.",
      "Ein freihändiger Agent für Linux: eine Wake-Word-Sprachschleife mit Speech-to-Text und gestreamter Sprachausgabe, MCP-Tool-Server für Shell, Browser, Dateisystem und Desktop-Steuerung sowie ein Gedächtnis über Gespräche hinweg.",
      "Un agente manos libres para Linux: un bucle de voz con palabra de activación, transcripción y voz en streaming, servidores de herramientas MCP para shell, navegador, sistema de archivos y control del escritorio, y memoria persistente entre conversaciones.",
      "Un agent mains libres pour Linux : une boucle vocale à mot d'activation, transcription et voix en streaming, des serveurs d'outils MCP pour le shell, le navigateur, le système de fichiers et le bureau, et une mémoire persistante entre les conversations.",
    ),
    impact: l(
      "A safety layer classifies every tool call as safe, confirm or dangerous before it runs. Open source on GitHub — the card opens the repository.",
      "Bezpečnostní vrstva každé volání nástroje předem zařadí jako bezpečné, k potvrzení, nebo nebezpečné. Open source na GitHubu — karta otevře repozitář.",
      "Слой безопасности классифицирует каждый вызов инструмента как безопасный, требующий подтверждения или опасный. Open source на GitHub — карточка открывает репозиторий.",
      "Eine Sicherheitsschicht stuft jeden Tool-Aufruf vorab als sicher, bestätigungspflichtig oder gefährlich ein. Open Source auf GitHub — die Karte öffnet das Repository.",
      "Una capa de seguridad clasifica cada llamada a herramienta como segura, a confirmar o peligrosa antes de ejecutarse. Código abierto en GitHub — la tarjeta abre el repositorio.",
      "Une couche de sécurité classe chaque appel d'outil comme sûr, à confirmer ou dangereux avant exécution. Open source sur GitHub — la carte ouvre le dépôt.",
    ),
    stack: ["TypeScript", "Node.js", "Claude Agent SDK", "MCP", "Zod", "Whisper STT"],
    accent: "violet",
  },
  {
    title: "Freio",
  ...
  {
    title: "Classio",
    href: "https://www.classio.cz/",
    image: "/projects/classio.png",
    eyebrow: l(
    ...
    description: l(
      "A school information system for Czech and Slovak schools, built as a replacement for Bakaláři except the state register: timetables, substitutions, weighted grades, attendance and absence notes, homework, messages, the class book, reports and printing — in one application for management, teachers, pupils and parents.",
      "Školní informační systém pro české a slovenské školy, postavený jako náhrada Bakalářů kromě školní matriky: rozvrh, suplování, známky s váhami, docházka a omluvenky, úkoly, zprávy, třídní kniha, vysvědčení i tisk — v jedné aplikaci pro vedení, učitele, žáky a rodiče.",
      "Школьная информационная система для чешских и словацких школ, задуманная как замена Bakaláři кроме государственного реестра: расписание, замены, оценки с весами, посещаемость и объяснительные, домашние задания, сообщения, классный журнал, табели и печать — в одном приложении для руководства, учителей, учеников и родителей.",
      "Ein Schulinformationssystem für tschechische und slowakische Schulen, gebaut als Ersatz für Bakaláři außer dem staatlichen Register: Stundenplan, Vertretungen, gewichtete Noten, Anwesenheit und Entschuldigungen, Hausaufgaben, Nachrichten, Klassenbuch, Zeugnisse und Druck — in einer Anwendung für Leitung, Lehrkräfte, Schüler und Eltern.",
      "Un sistema de información escolar para colegios checos y eslovacos, concebido como sustituto de Bakaláři salvo el registro estatal: horarios, sustituciones, notas ponderadas, asistencia y justificantes, deberes, mensajes, libro de clase, boletines e impresión — en una sola aplicación para dirección, profesores, alumnos y familias.",
      "Un système d'information scolaire pour les écoles tchèques et slovaques, conçu comme un remplacement de Bakaláři hors registre d'État : emplois du temps, remplacements, notes pondérées, présence et justificatifs, devoirs, messages, cahier de classe, bulletins et impression — dans une seule application pour la direction, les enseignants, les élèves et les parents.",
    ),
    impact: l(
      "Web app plus native iOS and Android clients, with the data kept on a server in Czechia and nothing to install on the school's own machines.",
      "Webová aplikace plus nativní klienti pro iOS a Android, data zůstávají na serveru v Česku a na školní server se nic neinstaluje.",
      "Веб-приложение плюс нативные клиенты для iOS и Android, данные остаются на сервере в Чехии, а на школьный сервер ничего не ставится.",
      "Web-App plus native iOS- und Android-Clients; die Daten bleiben auf einem Server in Tschechien, auf dem Schulserver wird nichts installiert.",
      "Aplicación web más clientes nativos de iOS y Android; los datos permanecen en un servidor en Chequia y no se instala nada en el servidor del colegio.",
      "Application web plus clients natifs iOS et Android ; les données restent sur un serveur en Tchéquie et rien ne s'installe sur le serveur de l'école.",
    ),
    stack: ["TypeScript", "Next.js", "PostgreSQL", "Flutter", "Docker"],
    accent: "green",
  },
  {
    title: "LaunchHub",
    href: "https://github.com/DanilaAnikin",
    image: "/projects/launchhub.png",
    eyebrow: l(
      "Internal control plane",
      "Vnitřní řídicí vrstva",
      "Внутренний control plane",
      "Interne Control Plane",
      "Plano de control interno",
      "Control plane interne",
    ),
    description: l(
      "The control plane I run my own products from: a web app, a background worker, a CLI and an MCP server sitting on one shared core, used to provision applications on the homelab and operate them afterwards.",
      "Řídicí vrstva, ze které provozuju vlastní produkty: webová aplikace, workerem na pozadí, CLI a MCP server nad jedním společným jádrem, kterým se aplikace na homelabu zakládají a pak spravují.",
      "Control plane, из которого я управляю собственными продуктами: веб-приложение, фоновый воркер, CLI и MCP-сервер поверх общего ядра — ими приложения на homelab создаются и затем обслуживаются.",
      "Die Control Plane, aus der ich meine eigenen Produkte betreibe: eine Web-App, ein Hintergrund-Worker, ein CLI und ein MCP-Server auf einem gemeinsamen Kern, mit dem Anwendungen auf dem Homelab angelegt und danach betrieben werden.",
      "El plano de control desde el que opero mis propios productos: una aplicación web, un worker en segundo plano, una CLI y un servidor MCP sobre un núcleo común, con el que se aprovisionan y luego se gestionan las aplicaciones del homelab.",
      "Le control plane depuis lequel je fais tourner mes propres produits : une application web, un worker de fond, une CLI et un serveur MCP sur un noyau commun, qui provisionne les applications du homelab puis les exploite.",
    ),
    impact: l(
      "Four entry points over one core, so the same operation behaves identically whether it is triggered by a person, a script or an agent.",
      "Čtyři vstupy nad jedním jádrem, takže stejná operace proběhne shodně, ať ji spustí člověk, skript, nebo agent.",
      "Четыре точки входа над одним ядром, поэтому одна и та же операция выполняется одинаково, кто бы её ни запустил — человек, скрипт или агент.",
      "Vier Einstiegspunkte über einem Kern, sodass dieselbe Operation identisch abläuft — ob von einem Menschen, einem Skript oder einem Agenten ausgelöst.",
      "Cuatro puntos de entrada sobre un mismo núcleo, de modo que la misma operación se comporta igual la lance una persona, un script o un agente.",
      "Quatre points d'entrée sur un seul noyau, si bien que la même opération se comporte à l'identique, qu'elle soit lancée par une personne, un script ou un agent.",
    ),
    stack: ["Next.js", "Hono", "Drizzle", "Turborepo", "Docker"],
    accent: "yellow",
  },
  {
    title: "Lokwave",
